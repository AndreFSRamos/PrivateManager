<template>
    ...
    <v-dialog v-model="dialog" persistent width="1024">
      ...
      <template v-slot:activator="{ props }">
        <v-btn v-bind="props">Novo Tipo de Gastos</v-btn>
      </template>

      <v-card>
        <v-card-title> <span class="text-h5">Adicionar Novo Tipo D Gasto</span> </v-card-title>
        <v-card-text>
          <v-container>
            <v-row>
...
data(){
  ...
  methods:{
      createSubmitForm() {
          let apiURL= 'http://localhost:5001/tipo_items';
